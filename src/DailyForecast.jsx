export default function DailyForecast({weather,error,loading}) {
    if (loading) {
        ...
        while (i <= 7) {
            items.push(<div className="bg-neutral-800 rounded flex flex-col gap-2 flex-1 min-w-[100px] py-2 h-30 animate-pulse">
        ...
        <div>
            ...
            <div className="flex gap-2">
                ...
                {weather.daily.time.map((day, index) => (
                    <div  key={day} className="bg-neutral-800 rounded flex flex-col gap-2 flex-1 min-w-[100px] py-2">
                        <p className="text-neutral-400">{new Date(day).toLocaleDateString("en-US", { weekday: "short" })}</p>
                        <img src={getWeatherIcon(weather.daily.weather_code[index])} className="w-25"/>
